    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                PackageInfo info;
                try {
                    info = getPackageManager().getPackageInfo("com.smartlab.drivetrain.license", 0);
                    ...
                    if (currentVersion > lastVersion) {
                        ...
                        intent = new Intent(Drive.this,WelCome.class);//应用程序首次启动时，将启动引导进入欢迎页面
                        startActivity(intent);
                        Drive.this.finish();
                        overridePendingTransition(R.anim.fade_in,R.anim.fade_out);
                    } else {
                        intent = new Intent(Drive.this,MainContent.class);//应用程序首次启动时，将启动引导进入欢迎页面
                        startActivity(intent);
                        Drive.this.finish();
                        overridePendingTransition(R.anim.fade_in,R.anim.fade_out);
                    }
